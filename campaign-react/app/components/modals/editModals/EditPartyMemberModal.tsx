...
const EditPartyMemberModal = ( { member } : { member:PartyMemberType}) => {
    ...
    useEffect(() => {
        ...
        setJoinDate(String(member.join_date))
        setLeaveDate(String(member.leave_date))
    }, [member]);
    ...
    const editPartyMember = async () => {
        ...
        const response = await CampaignJournal.update(
            `/campaigncore/${campaign_id}/party/${entryID}/`,
        ...
        if (response.id) {         
            ...
            setSuccessMessage("Party Member Has Been Updated"); //Success Modal
...
export default EditPartyMemberModal;