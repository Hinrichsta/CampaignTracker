...
class IsCampaignOwner(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        try:
            campaign_user = CampaignUsers.objects.get(user=request.user, campaign=view.get_object())
            return campaign_user.role == 'O'
...
class IsCampaignAdmin(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        try:
            campaign_user = CampaignUsers.objects.get(user=request.user, campaign=view.get_object())
            return campaign_user.role in ['S', 'O', 'A']
        except CampaignUsers.DoesNotExist:
            return False
